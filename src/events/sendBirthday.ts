...
async function sendBirthdayMessage(client: Client<true>, channel: string) {
  const { currMonth, currDay } = getCurrentDate();
  const birthdays = await getBirthdaysByMonthDay(currMonth, currDay);
  // const birthdays = [{ userid: '894019356705562674', month: 1, day: 1 }];
  ...
  for (const birthday of birthdays) {
    ...
    var isActive = false;
    for (const role_id_str of process.env.ACTIVE_ROLES.split(',')) {
      const role = guild.members.cache.get(userid).roles.cache.get(role_id_str);
      console.log(`Checking role ${role_id_str} for user ${user}:`, role?.id);
      if (role !== undefined) {
        isActive = true;
        break;
      }
    ...
    if (!isActive) {
      console.log(`Skipping sending happy birthday to ${user} on month: ${month}, day: ${day} -- user is not active`);
      continue;
    }
...
export default {
  ...
  async execute(client: Client<true>) {
    // sendBirthdayMessage(client, CHANNEL_GENERAL);
