...
export function makeAppreciateEmbed(options: AppreciateEmbedOptions) {
  ...
  if (options.appreciator) embed.addField('Appreciator', `<@${options.appreciator}>`, true);
  if (options.appreciated) embed.addField('Appreciated', `<@${options.appreciated}>`, true);
  else embed.addField('Appreciated', 'ACE', true);
  if (options.appreciationMessage) {
    embed.addField('Message', options.appreciationMessage);
  } else {
    embed.addField('Message', `Thanks for being amazing and brightening someone's day!`);
  }
