  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName('replyto')
        .setDescription(
          'Specify this option to turn the message into a reply, this is the message that ECA should reply to'
        )
        .setRequired(false)
    )
    .addBooleanOption((option) =>
      option
        .setName('replyping')
        .setDescription(
          'If ECA is replying to a message, whether the reply should ping the original author. Defaults to true'
        )
        .setRequired(false)
    ),
